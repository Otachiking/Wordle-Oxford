...
function getEmojiForFallback(word, def) {
    ...
    // some simple heuristic mappings
                ...
                if (res.statusCode === 200) {
                    try {
                        ...
                        if (meanings && meanings.length > 0) {
                            ...
                            // Shorten the description contextually
                            if (def.length > 70) {
                                // try to cut it at a space around 60 chars
                            ...
                            // Clean up trailing punctuation before ellipsis if we added it, but just return what we have
                            // if it's naturally short.
                            resolve(def);
                            return;
                        }
                    } catch(e) {}
...
async function run() {
    console.log("Updating next 800 words (index 200 to 999)...");
    let count = 0;
    
    // We already updated 0-199 from the previous script runs!
    for (let i = 200; i < 1000; i++) {
        if (!data[i]) break; // prevent out of bounds
        
        const word = data[i].word;
        process.stdout.write(`Progress: ${i-200}/800 (${word})      \r`);
        ...
        if (def) {
            ...
            // retain existing definition or mark it
