...
async function run() {
    console.log("Waiting for the previous update (1000-1999) to finish saving...");
    
    // Polling
    while (true) {
        const currentData = JSON.parse(fs.readFileSync(path, 'utf8'));
        // check if index 1999 was successfully updated by the previous script
        if (currentData[1999] && currentData[1999].definition && currentData[1999].definition !== "") {
            console.log("Previous script finished. Proceeding with 2000-2999...");
            break;
        }
    ...
    console.log("Updating next 1000 words (index 2000 to 2999)...");
    
    for (let i = 2000; i < 3000; i++) {
        if (!data[i]) break;
        
        const word = data[i].word;
        process.stdout.write(`Progress: ${i-2000}/1000 (${word})      \r`);
        ...
        // Wait 100ms before next request to avoid limit
